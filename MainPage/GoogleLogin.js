...
function iniciarLoginGoogle() {
    fetch('client_secret_182308303226-9ma1qn7f10b7irrfajak3p4lv5op5tal.apps.googleusercontent.com.json')
        ...
        .then(data => {
            ...
            const redirectUri = data.web.redirect_uris[0];
    ...
    fetch(backendUrl, {
        ...
        .then(data => {
            ...
            if (data.id_token) {
                ...
                // Salvar o access token no localStorage e recarregar a página
                localStorage.setItem('accessToken', data.access_token);
                window.location.search = ""; // Remove o código de autorização da URL
                window.location.reload();
            } else {
                console.error("Erro ao obter idToken:", data);
            }
        })
        .catch(error => console.error("Erro ao obter idToken:", error));
}
